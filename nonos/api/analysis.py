...
from nonos.api._geometry import (
    Axis,
    Coordinates,
    Geometry,
    axes_from_geometry,
...
class Plotable:
    ...
    def plot(
        ...
        **kwargs,
        ...
        def _str_axis_key(axis_or_str: Union[Axis, str]) -> str:
            if isinstance(axis_or_str, Axis):
                return axis_to_str(axis_or_str)
            else:
                return axis_or_str
        ...
        if self.dimension == 2:
            ...
            ax.set_xlabel(_str_axis_key(self.akey))
            ax.set_ylabel(_str_axis_key(self.okey))
...
class GasField:
    ...
    def map(
        ...
        *wanted,
        ...
        if dimension == 1:
            ...
            axis_1 = str_to_axis(axis_1_str)
            axis_2 = str_to_axis(axis_2_str)
    ...
    def vertical_projection(self, z=None, *, operation_name=None) -> "GasField":
        ...
        if self.native_geometry is Geometry.CARTESIAN:
            ...
            ret_coords = self.coords.project_along(Axis.CARTESIAN_Z, zmed[imid])
    ...
    def vertical_at_midplane(self, *, operation_name=None) -> "GasField":
        ...
        if self.native_geometry is Geometry.CARTESIAN:
            ...
            ret_coords = self.coords.project_along(Axis.COLATITUDE, thetamed[imid])
    ...
    def azimuthal_at_phi(self, phi=0.0, *, operation_name=None) -> "GasField":
        ...
        if self.native_geometry is Geometry.CARTESIAN:
            ...
            ret_coords = self.coords.project_along(Axis.AZIMUTH, phimed[iphi])
    ...
    def radial_at_r(self, distance=1.0, *, operation_name=None) -> "GasField":
        ...
        if self.native_geometry is Geometry.CARTESIAN:
            ...
            ret_coords = self.coords.project_along(Axis.CYLINDRICAL_RADIUS, rmed[ir1])
        elif self.native_geometry is Geometry.SPHERICAL:
            rmed = self.coords.get_axis_array_med(Axis.SPHERICAL_RADIUS)
            ret_coords = self.coords.project_along(Axis.SPHERICAL_RADIUS, rmed[ir1])
        else:
            assert_never(self.native_geometry)
    ...
    def radial_average_interval(
        ...
        if self.native_geometry is Geometry.CARTESIAN:
            ...
            if vmax is None:
                vmax = R.max()
            Rmed = self.coords.get_axis_array_med(Axis.CYLINDRICAL_RADIUS)
            ret_coords = self.coords.project_along(Axis.CYLINDRICAL_RADIUS, Rmed[ir])
        elif self.native_geometry is Geometry.SPHERICAL:
            ...
            if vmax is None:
                vmax = r.max()
            rmed = self.coords.get_axis_array_med(Axis.SPHERICAL_RADIUS)
            ret_coords = self.coords.project_along(Axis.SPHERICAL_RADIUS, rmed[ir])
        else:
            assert_never(self.native_geometry)
