...
def axis_to_str(axis: Axis) -> str:
    return _AXIS_TO_STR[axis]


def str_to_axis(s: str) -> Axis:
    return _STR_TO_AXIS[s]
...
def _native_plane_from_target_plane(
    ...
    # Chosing a target plane requires 2 axes, which gives 3 possible planes
    # per target geometry => 3^3 = 27 (order insensive) cases
...
def _deprecated_axis_array_property(attr: str, replacement: str):
    def _wrapped(self) -> FloatArray:
        warnings.warn(
            f"Coordinates.{attr} is deprecated. "
            f"Instead, use Coordinates.{replacement}({attr!r})",
            DeprecationWarning,
            stacklevel=2,
        )
        try:
            return getattr(self, replacement)(attr)
        except KeyError:
...
class Coordinates:
    ...
    x = _deprecated_axis_array_property("x", "get_axis_array")
    y = _deprecated_axis_array_property("y", "get_axis_array")
    z = _deprecated_axis_array_property("z", "get_axis_array")
    r = _deprecated_axis_array_property("r", "get_axis_array")
    theta = _deprecated_axis_array_property("theta", "get_axis_array")
    phi = _deprecated_axis_array_property("phi", "get_axis_array")
    R = _deprecated_axis_array_property("R", "get_axis_array")

    xmed = _deprecated_axis_array_property("x", "get_axis_array_med")
    ymed = _deprecated_axis_array_property("y", "get_axis_array_med")
    zmed = _deprecated_axis_array_property("z", "get_axis_array_med")
    rmed = _deprecated_axis_array_property("r", "get_axis_array_med")
    thetamed = _deprecated_axis_array_property("theta", "get_axis_array_med")
    phimed = _deprecated_axis_array_property("phi", "get_axis_array_med")
    Rmed = _deprecated_axis_array_property("R", "get_axis_array_med")
    ...
    def to_dict(self) -> StrDict:
        return {"geometry": self.geometry} | {
            axis_to_str(axis): self.get_axis_array(axis) for axis in self.axes
        }

    def native_from_wanted(self, axis_1: Axis, axis_2: Optional[Axis] = None, /):
    ...
    def _meshgrid_conversion_1d(self, axis_1: Axis) -> dict:
    ...
    def _meshgrid_conversion_2d(self, axis_1: Axis, axis_2: Axis) -> dict:
        # TODO: fix interface
    ...
    def _meshgrid_reduction(self, axis_1: Axis, axis_2: Optional[Axis], /) -> dict:
        # TODO: this could easily be splitted into 2 functions (one for each dimensions)
        axes = axis_1, axis_2
